...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  static const String _audioTimerPoolKey =
      'home_page_audio_timers'; // 独立的音频定时器池
  ...
  final TextEditingController _sentenceInputController =
  ...
  Future<void> _loadWordsFromSelectedWordBook() async {
    try {
      ...
      // 从缓存加载词库数据
      final wordData =
          await CacheService.getCachedWordData(selectedWordBookName);
  ...
  Future<void> _generateNextWord() async {
    ...
    try {
      ...
      // 使用智能推荐算法获取推荐单词（优先复习）
      final recommendedWords =
          await LearningDataService.instance.getRecommendedWords(
      ...
      // 获取需要复习的单词数量（用于调试）
      final reviewWords = await LearningDataService.instance
          .getReviewWords(_currentWordBookName!);
      ...
      _currentWord = await ExtendedWordData.fromWordData(
          selectedWordData, pronunciationType);
    } catch (e) {
      // 如果推荐算法出错，回退到随机选择
      ...
      final pronunciationType = await SettingsHelper.getPronunciationType();
      _currentWord =
          await ExtendedWordData.fromWordData(wordData, pronunciationType);
    }
  }
  ...
  void _initializeCharacterAnimations() {
    ...
    _wordSlideAnimations = _wordControllers
        .map(
          (controller) =>
              PerformanceOptimizer.getTween<double>('slide_tween', 12.0, 0.0)
                  .animate(
    ...
    _wordOpacityAnimations = _wordControllers
        .map(
          (controller) =>
              PerformanceOptimizer.getTween<double>('opacity_tween', 0.0, 1.0)
                  .animate(
            PerformanceOptimizer.getCurvedAnimation(
              parent: controller,
              curve: Curves.easeOutCubic,
    ...
    _translationSlideAnimations = _translationControllers
        .map(
          (controller) =>
              PerformanceOptimizer.getTween<double>('slide_tween', 12.0, 0.0)
                  .animate(
    ...
    _translationOpacityAnimations = _translationControllers
        .map(
          (controller) =>
              PerformanceOptimizer.getTween<double>('opacity_tween', 0.0, 1.0)
                  .animate(
    ...
    _exampleSlideAnimations = _exampleControllers
        .map(
          (controller) =>
              PerformanceOptimizer.getTween<double>('slide_tween', 12.0, 0.0)
                  .animate(
    ...
    _exampleOpacityAnimations = _exampleControllers
        .map(
          (controller) =>
              PerformanceOptimizer.getTween<double>('opacity_tween', 0.0, 1.0)
                  .animate(
    ...
    _exampleTranslationSlideAnimations = _exampleTranslationControllers
        .map(
          (controller) =>
              PerformanceOptimizer.getTween<double>('slide_tween', 12.0, 0.0)
                  .animate(
    ...
    _exampleTranslationOpacityAnimations = _exampleTranslationControllers
        .map(
          (controller) =>
              PerformanceOptimizer.getTween<double>('opacity_tween', 0.0, 1.0)
                  .animate(
  ...
  void _onWordAnimationCompleted() {
    if (mounted) {
      ...
      PerformanceOptimizer.createTimer(
        ...
        callback: () {
          if (mounted &&
              _buttonsController.status != AnimationStatus.completed) {
  ...
  void _prepareCharacterAnimations(ExtendedWordData word) {
    ...
    _wordSlideAnimations = _wordControllers
        .map(
          (controller) => Tween<double>(begin: 15.0, end: 0.0).animate(
            CurvedAnimation(
              parent: controller,
              curve: Curves.easeOutQuart,
            ),
    ...
    _translationSlideAnimations = _translationControllers
        .map(
          (controller) => Tween<double>(begin: 12.0, end: 0.0).animate(
            CurvedAnimation(
              parent: controller,
              curve: Curves.easeOutQuart,
            ),
    ...
    _exampleTranslationSlideAnimations = _exampleTranslationControllers
        .map(
          (controller) => Tween<double>(begin: 10.0, end: 0.0).animate(
            CurvedAnimation(
              parent: controller,
              curve: Curves.easeOutQuart,
            ),
  ...
  void _startCharacterAnimation(
      List<AnimationController> controllers, int delayMs) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            title: AnimatedSwitcher(
              ...
              child: Text(
                (_isTestingMode || _isSentenceSubmitted)
                    ? _currentWord?.word ?? 'WordFlow'
                    : 'WordFlow',
                key: ValueKey((_isTestingMode || _isSentenceSubmitted)
                    ? _currentWord?.word
                    : 'WordFlow'),
            ...
            leading: (!_isTestingMode && !_isSentenceSubmitted)
                ? Padding(
                    ...
                    child: Theme(
                      ...
                      child: IconButton(
                        ...
                        iconSize:
                            ResponsiveHelper.getResponsiveIconSize(context, 26),
                        onPressed: () async {
              ...
              ElevatedButton.icon(
                ...
                style: ElevatedButton.styleFrom(
                  ...
                  padding: EdgeInsets.symmetric(
                      horizontal: 20, vertical: 10), // 从24,12减少到20,10
                ),
              ),
            ],
          ),
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // 底部浅灰字提示
                    AnimatedOpacity(
                      opacity: (!_isTestingMode && _wordAnimationCompleted)
                          ? 0.6
                          : 0.0,
                      duration: const Duration(milliseconds: 800),
                      child: Text(
                    ...
                    SizedBox(
                        height:
                            ResponsiveHelper.getResponsiveSpacing(context, 60)),
                  ],
                ),
              ),
            ),
  ...
  void _markAsKnown() async {
    ...
    if (learningMode == LearningMode.quickMemory) {
      ...
      // 深入学习模式：检查 AI 接口配置是否可用
      final apiKey = await DeepSeekApiService.getApiKey();
      final hasUsableAiConfig =
          apiKey != null && apiKey.isNotEmpty && apiKey.length >= 10;

      if (!hasUsableAiConfig) {
        // AI 配置无效，自动切换到快速学习模式并显示提示
        await SettingsHelper.setLearningMode(LearningMode.quickMemory);

        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('AI 接口配置无效，已自动切换到快速学习模式'),
  ...
  Future<void> _undoLearningRecord(String wordToUndo) async {
    ...
    final existingRecord =
        records.where((r) => r.word == wordToUndo).firstOrNull;

    if (existingRecord != null && existingRecord.reviewHistory.isNotEmpty) {
      // 创建新的复习历史列表，删除最后一条记录
      final newReviewHistory =
          List<ReviewRecord>.from(existingRecord.reviewHistory);
      newReviewHistory.removeLast();

      if (newReviewHistory.isEmpty) {
        // 如果没有复习历史了，删除整个记录
        await LearningDataService.instance
            .removeWordLearningRecord(wordToUndo, _currentWordBookName!);
      } else {
        // 恢复到上一个状态
        ...
        for (final review in newReviewHistory) {
          switch (review.reviewResult) {
            ...
            case ReviewResult.good:
              recalculatedMemoryLevel =
                  recalculatedMemoryLevel.index < MemoryLevel.values.length - 1
                      ? MemoryLevel.values[recalculatedMemoryLevel.index + 1]
                      : MemoryLevel.mastered;
              break;
            case ReviewResult.easy:
              recalculatedMemoryLevel =
                  recalculatedMemoryLevel.index < MemoryLevel.values.length - 2
        ...
        final updatedRecord = WordLearningRecord(
          ...
          nextReviewTime: previousReview.reviewTime
              .add(Duration(days: previousReview.reviewInterval.round())),
        ...
        // 保存更新后的记录
        await LearningDataService.instance
            .saveWordLearningRecord(updatedRecord);
      }
  ...
  Future<void> _saveLearningRecord(ReviewResult result) async {
    ...
    final records = await LearningDataService.instance
        .getWordBookRecords(_currentWordBookName!);
    final existingRecord =
        records.where((r) => r.word == _currentWord!.word).firstOrNull;
  ...
  void _initializeBetterSentenceAnimation() {
    ...
    // 创建动画
    _betterSentenceAnimations = _betterSentenceControllers
        .map(
          (controller) => Tween<double>(begin: 0.0, end: 1.0).animate(
            CurvedAnimation(
              parent: controller,
                                ...
                                children: [
                                  ...
                                  FutureBuilder<Map<String, dynamic>>(
                                    ...
                                    builder: (context, snapshot) {
                                      ...
                                      final hasAudio =
                                          snapshot.data!['hasAudio'] as bool;
                            ...
                            FutureBuilder<Map<String, dynamic>>(
                              ...
                              builder: (context, snapshot) {
                                final hasPhonetic = snapshot.hasData &&
                                    (snapshot.data!['phonetic'] as String)
                                        .isNotEmpty;
                                ...
                                return Column(
                                  children: [
                                    SizedBox(
                                        height: hasPhonetic
                                            ? 2
                                            : 8), // 有音标时间距为2，无音标时统一增加到8
                                    _buildPhoneticSection(word),
                                    if (hasPhonetic)
                                      const SizedBox(
                                          height: 8), // 音标和释义之间的间距统一为8
                                  ],
                                );
                              },
                            ),
        ...
        child: Center(
          child: Wrap(
            ...
            children: List.generate(text.length, (index) {
              ...
              if (index >= slideAnimations.length ||
              ...
              return RepaintBoundary(
                child: AnimatedBuilder(
                  animation: Listenable.merge(
                      [slideAnimations[index], opacityAnimations[index]]),
                  builder: (context, child) {
                    return Transform.translate(
  ...
  double _measureTextWidth(String text, TextStyle style) {
    // 创建缓存键，包含文本和样式信息
    final cacheKey =
        '${text}_${style.fontSize}_${style.fontFamily}_${style.fontWeight}';
  ...
  Widget _buildWordLevelAnimatedText(
    ...
    return Center(
      child: ConstrainedBox(
        ...
        child: _buildWordAnimatedWrap(
            words, slideAnimations, opacityAnimations, style),
      ),
    );
  }
    ...
    return Center(
      child: ConstrainedBox(
        ...
        child: Column(
          ...
          children: _buildWordBasedAnimatedLines(
              text, slideAnimations, opacityAnimations, style),
        ),
      ),
    );
  }
  ...
  List<Widget> _buildWordBasedAnimatedLines(
    ...
    for (int lineIndex = 0; lineIndex < lines.length; lineIndex++) {
      ...
      if (!isMostlyChinese && lineIndex < lines.length - 1) {
        // 检查原文本中是否有空格需要考虑
        final totalCharsInPreviousLines = lines
            .take(lineIndex + 1)
            .map((l) => l.length)
            .fold(0, (a, b) => a + b);
  ...
  Widget _buildAnimatedTextLine(
    ...
    return Wrap(
      ...
      children: List.generate(line.length, (localIndex) {
        ...
        if (globalIndex >= slideAnimations.length ||
            globalIndex >= opacityAnimations.length) {
          // 使用最后一个动画的值，或者直接显示
        ...
        return AnimatedBuilder(
          animation: Listenable.merge(
              [slideAnimations[globalIndex], opacityAnimations[globalIndex]]),
          builder: (context, child) {
            return Transform.translate(
          ...
          return Transform.translate(
            ...
            child: Opacity(
              ...
              child: AnimatedSwitcher(
                ...
                child: OptimizedText(
                  _isTestingMode
                      ? (_showSentenceInput ? '' : '正在准备造句测试...')
                      : (_wordAnimationCompleted ? '' : '单词即将登场...'),
                  key: ValueKey(
                      '${_wordAnimationCompleted}_${_isTestingMode}_breath'),
                  style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                        color: Theme.of(context).brightness == Brightness.dark
                            ? Theme.of(context)
                                .textTheme
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color:
                        isDark ? AppTheme.darkCardColor : AppTheme.coolGray100,
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(
                        color: isDark
                            ? AppTheme.darkCardColor
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          padding: const EdgeInsets.symmetric(
                              horizontal: 16, vertical: 10), // 与弹出框完全一致的padding
                          decoration: BoxDecoration(
                            color:
                                Theme.of(context).brightness == Brightness.dark
                                    ? AppTheme.darkCardColor
                                    : AppTheme.coolGray50,
                            borderRadius:
                                BorderRadius.circular(12), // 与弹出框完全一致的圆角
                            border: Border.all(
                              color: Theme.of(context).brightness ==
                                      Brightness.dark
                                  ? AppTheme.coolGray600
                ...
                Column(
                  children: [
                    ...
                    AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        return Opacity(
                          opacity: _isSentenceSubmitted
                              ? _sendButtonFadeAnimation.value
                              : 1.0,
                          child: SizedBox(
                            width: double.infinity,
                    ...
                    AnimatedBuilder(
                      ...
                      builder: (context, child) {
                        return Opacity(
                          opacity:
                              _isJudging ? _skipButtonFadeAnimation.value : 1.0,
                          child: TextButton(
                            onPressed: _isJudging
                                ? null
                                : () {
  ...
  Widget _buildSentenceResult() {
    if (_judgmentResult == null || _userSentence.isEmpty) {
  ...
  Widget _buildHighlightedSentence() {
    ...
    return Container(
      ...
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: AppTheme.getSecondaryTitleColor(context,
            lightColor: AppTheme.coolGray50),
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: AppTheme.getSecondaryTitleColor(context,
              lightColor: AppTheme.coolGray200),
        ),
        boxShadow: isDark
            ? null
            : [
      ...
      child: Text.rich(
        TextSpan(
          children: words.map((word) {
            // 智能错误检测逻辑
            Color textColor =
                isDark ? AppTheme.darkPrimaryTextColor : AppTheme.coolGray700;
  ...
  /// 检查单词是否在错误描述中被提及
  bool _checkWordInErrorDescription(
      String word, SentenceJudgmentResult result) {
  ...
  bool _isWordDifferentFromCorrect(String userWord, String correctSentence) {
    ...
    final userWordClean =
        userWord.toLowerCase().replaceAll(RegExp(r'[^\w]'), '');
    final correctWordClean =
        correctWords[wordIndex].toLowerCase().replaceAll(RegExp(r'[^\w]'), '');
    ...
    // 检查语法结构错误（通过检查前后文）
    return _isWordInWrongGrammarContext(
        userWord, wordIndex, userWords, correctWords);
  }

  /// 检查单词是否在错误的语法环境中
  bool _isWordInWrongGrammarContext(String userWord, int wordIndex,
      List<String> userWords, List<String> correctWords) {
    // 检查常见的语法错误模式
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          // 逐字母浮现的正确句子
          if (_showBetterSentenceAnimation &&
              _betterSentenceText.isNotEmpty) ...[
          ...
          Row(
            children: [
              Icon(
                ...
                color: hasErrors
                    ? AppTheme.getSecondaryTitleColor(context,
                        lightColor: AppTheme.coolGray600)
                    : AppTheme.accentGreen,
                size: 18,
              ),
              const SizedBox(width: 8),
              RenderCompatibilityHelper.createCompatibleText(
                ...
                style: TextStyle(
                  ...
                  color: AppTheme.getPrimaryTitleColor(context,
                      lightColor: AppTheme.coolGray700),
                ),
              ),
            ],
          ),
        ...
        children: List.generate(words.length, (index) {
          ...
          if (index >= _betterSentenceAnimations.length) {
            return RenderCompatibilityHelper.createCompatibleText(
              ...
              style: TextStyle(
                ...
                fontWeight: modificationInfo.isModified
                    ? FontWeight.w600
                    : FontWeight.w500,
                height: 1.5,
              ),
          ...
          return AnimatedBuilder(
            ...
            builder: (context, child) {
              return Transform.translate(
                offset:
                    Offset(0, 8 * (1 - _betterSentenceAnimations[index].value)),
                child: Opacity(
  ...
  WordModificationInfo _getWordModificationInfo(String correctWord, int index) {
    ...
    if (index >= userWords.length) {
      return WordModificationInfo(
          isModified: true, modificationType: ModificationType.grammar);
    }
    ...
    if (_isWordInSimplicityImprovement(correctWord, index)) {
      return WordModificationInfo(
          isModified: true, modificationType: ModificationType.simplicity);
    }

    // 检查是否是地道性修改
    if (_isWordInIdiomaticImprovement(correctWord, index)) {
      return WordModificationInfo(
          isModified: true, modificationType: ModificationType.idiomatic);
    }

    // 检查是否在语法修正的范围内
    if (_isWordInCorrectedGrammarRange(
        correctWord, index, userWords, correctWords)) {
      return WordModificationInfo(
          isModified: true, modificationType: ModificationType.grammar);
    }

    return WordModificationInfo(
        isModified: false, modificationType: ModificationType.none);
  }
  ...
  bool _isWordInSimplicityImprovement(String correctWord, int index) {
    ...
    if (userWords.length == correctWords.length && index < userWords.length) {
      final userWord =
          userWords[index].toLowerCase().replaceAll(RegExp(r'[^\w]'), '');
      final correctWordClean =
          correctWord.toLowerCase().replaceAll(RegExp(r'[^\w]'), '');
  ...
  Color _getWordHighlightColor(WordModificationInfo info) {
    if (!info.isModified) {
      return AppTheme.getPrimaryTitleColor(context,
          lightColor: AppTheme.coolGray700);
    }
    ...
    switch (info.modificationType) {
      ...
      case ModificationType.idiomatic:
        ...
        return isDark
            ...
            : Colors.orange.shade600; // 橙色表示地道性改进
      case ModificationType.simplicity:
        final isDark = Theme.of(context).brightness == Brightness.dark;
        return isDark
            ? Colors.amber.shade300
            : Colors.amber.shade600; // 黄色表示简单性改进
  ...
  /// 检查单词是否在语法修正的范围内
  bool _isWordInCorrectedGrammarRange(String correctWord, int index,
      List<String> userWords, List<String> correctWords) {
    ...
    for (int i = 0; i < userWords.length - 3; i++) {
      if (userWords[i].toLowerCase() == 'i' &&
        ...
        if (index >= i && index <= i + 5) {
          // 扩展范围包括"him very much"
          final currentWordLower = correctWord.toLowerCase();
          if (currentWordLower == 'i' ||
              currentWordLower == 'like' ||
              currentWordLower == 'him' ||
              currentWordLower == 'very' ||
...
class ExtendedWordData {
  ...
  static Future<ExtendedWordData> fromWordData(
      WordData wordData, PronunciationType pronunciationType) async {
    // 尝试从API获取详细信息
    final apiResponse =
        await EnglishWordApiService.getWordDetails(wordData.word);
    ...
    if (apiResponse != null) {
      ...
      if (example.isEmpty || exampleTranslation.isEmpty) {
        final aiExample =
            await _generateAIExample(wordData.word, mainTranslation);
      ...
      // 使用AI生成例句
      final aiExample =
          await _generateAIExample(wordData.word, wordData.translation);
  ...
  /// 使用AI生成例句
  static Future<ExampleSentenceResult> _generateAIExample(
