...
class AnimatedTextBuilder {
  ...
  const AnimatedTextBuilder({required this.controller});
  
  /// 构建标题文字
  Widget title(String text, {Duration delay = Duration.zero}) {
    return AnimatedTextHelper.buildAnimatedText(
      text: text,
      style: AnimatedTextStyles.title,
  ...
  /// 构建副标题文字
  Widget subtitle(String text, {Duration delay = Duration.zero}) {
    return AnimatedTextHelper.buildAnimatedText(
      text: text,
      style: AnimatedTextStyles.subtitle,
      animationController: controller,
      animationDelay: delay,
  ...
  /// 构建正文文字
  Widget body(String text, {Duration delay = Duration.zero}) {
    return AnimatedTextHelper.buildAnimatedText(
      text: text,
      style: AnimatedTextStyles.body,
      animationController: controller,
      animationDelay: delay,
      characterDelay: const Duration(milliseconds: 40),
      animationDistance: 15.0,
    );
  }
  
  /// 构建小字体文字
  Widget caption(String text, {Duration delay = Duration.zero}) {
    return AnimatedTextHelper.buildAnimatedText(
      text: text,
      style: AnimatedTextStyles.caption,
      animationController: controller,
      animationDelay: delay,
      characterDelay: const Duration(milliseconds: 30),
      animationDistance: 10.0,
    );
  }
} 